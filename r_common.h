...
enum GeometryType { GEOM_TYPE_VERTEX_ONLY, GEOM_TYPE_INDEXED };
...
struct ShaderSettings {
	glm::uvec4 u32bitMasks; // TODO: This is just to make the Shader happy
							// (Wants 16 bytes by default, not only 4).
};
...
// Representation of a Quad but only stores the four cornerpoints, not whole
// tris.
struct FaceQuad {
	union {
		struct {
...
Quad CreateQuad(glm::vec3 pos = glm::vec3(0, 0, 0), float width = 1.0f, float height = 1.0f,

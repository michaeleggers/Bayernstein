...
bool GLRender::Init() {
	...
	*/
	...
	m_Window = SDL_CreateWindow(
		"HKD", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WINDOW_WIDTH,
		WINDOW_HEIGHT,
	...
	// With sizeof(Vertex) = 92bytes => sizeof(Tri) = 276bytes we need ~ 263MB
	// for Models. A lot for a game in the 2000s! Our models have a tri count of
	// maybe 3000 Tris (without weapon), which is not even close to 1Mio tris.
...
void GLRender::RegisterColliderModels() {
	...
	m_EllipsoidColliderDrawCmd =
		AddTrisToBatch(m_ColliderBatch, unitEllipsoid.tris.data(),
					   unitEllipsoid.tris.size(), false, DRAW_MODE_WIREFRAME);
}

// Maybe return a void* as GPU handle, because usually APIs that use the handle
// of a specific graphics API don't expect it to be in int or whatever.
...
void GLRender::ImDrawTriPlanes(TriPlane *triPlanes, uint32_t numTriPlanes,
							   bool cullFace, DrawMode drawMode) {
	std::vector<Tri> tris;
...
GLBatchDrawCmd GLRender::AddTrisToBatch(GLBatch *batch, Tri *tris,
...
void GLRender::ImDrawIndexed(Vertex *verts, uint32_t numVerts,
							 uint16_t *indices, uint32_t numIndices,
							 bool cullFace, DrawMode drawMode) {
	int offset = 0;
	int offsetIndices = 0;
	if (!m_ImPrimitiveBatchIndexed->Add(verts, numVerts, indices, numIndices,
										&offset, &offsetIndices, cullFace,
										drawMode)) {
		return;
	}
...
void GLRender::ImDrawSphere(glm::vec3 pos, float radius, glm::vec4 color) {
	...
	glDrawArrays(GL_TRIANGLES, m_EllipsoidColliderDrawCmd.offset,
...
void GLRender::ExecuteDrawCmds(std::vector<GLBatchDrawCmd> &drawCmds,
	...
	for (auto drawCmd : drawCmds) {
		...
		if (geomType == GEOM_TYPE_VERTEX_ONLY) {
			...
			uint32_t indexBufferUSOffset =
				drawCmd.indexOffset * sizeof(uint16_t);
			glDrawElementsBaseVertex(
				primitiveType, drawCmd.numIndices, GL_UNSIGNED_SHORT,
...
void GLRender::Render(Camera *camera, HKD_Model **models, uint32_t numModels) {
	...
	//     glDrawArrays(GL_TRIANGLES, 3*modelDrawCmds[i].offset, 3 *
	//     modelDrawCmds[i].numTris);
	// }
	// glDrawArrays(GL_TRIANGLES, 0, 3*m_ModelBatch->TriCount());
}

void GLRender::RenderColliders(Camera *camera, HKD_Model **models,
							   uint32_t numModels) {
	glm::mat4 view = camera->ViewMatrix();
	// TODO: Global Setting for perspective values
	glm::mat4 proj = glm::perspective(
...
void GLRender::InitShaders() {
	...
	if (!m_ModelShader->Load("shaders/entities.vert", "shaders/entities.frag",
	...
	if (!m_ImPrimitivesShader->Load("shaders/primitives.vert",
	...
	if (!m_ColliderShader->Load("shaders/colliders.vert",
								"shaders/colliders.frag")) {
		printf("Problems initializin colliders shader!\n");
	}

	// TODO: Just to test if shaders overwrite data from each other. Delete
	// later!
	auto *foo = new Shader();
	if (!foo->Load("shaders/entities.vert", "shaders/entities.frag",
				   SHADER_FEATURE_MODEL_ANIMATION_BIT)) {
		printf("Problems initializing model shaders!\n");
	}
}

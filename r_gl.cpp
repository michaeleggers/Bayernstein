...
void GLRender::ImDrawCircle(glm::vec3 center, float radius, glm::vec3 normal) {
    ...
    glm::mat3 rotationQuat = glm::rotate(glm::mat4(1), angle, glm::normalize(axis));

    circleVerts.reserve(segments);
    for ( int i = 0; i < segments; i++ ) {
        glm::vec3 position = glm::vec3(radius * glm::cos(i * step), radius * glm::sin(i * step), 0.0f);
        position           = center + rotationQuat * position;
    ...
    Vertex endAndStart[] = { *v, circleVerts[ 0 ] };
    m_ImPrimitiveBatch->Add(endAndStart, 2, false, DRAW_MODE_LINES);
    moreVerts += 2;

    GLBatchDrawCmd drawCmd
        = { .offset = offset, .numVerts = segments + moreVerts, .cullFace = false, .drawMode = DRAW_MODE_LINES };

...
bool Shader::Load(const std::string &vertName, const std::string &fragName,
	...
	m_ViewProjUniformIndex =
		glGetUniformBlockIndex(m_ShaderProgram, "ViewProjMatrices");
	if (m_ViewProjUniformIndex == GL_INVALID_INDEX) {
		printf("SHADER-WARNING: Not able to get index for UBO in shader "
			   "program.\nShaders:\n %s\n %s\n",
			   vertName.c_str(), fragName.c_str());
		// TODO: What to do in this case???
	}
	glUniformBlockBinding(m_ShaderProgram, m_ViewProjUniformIndex,
						  bindingPoint);
	glBindBufferRange(GL_UNIFORM_BUFFER, bindingPoint, g_ViewProjUBO, 0,
					  2 * sizeof(glm::mat4));
	...
	m_SettingsUniformIndex =
		glGetUniformBlockIndex(m_ShaderProgram, "Settings");
	...
	glUniformBlockBinding(m_ShaderProgram, m_SettingsUniformIndex,
						  settingsBindingPoint);
	glBindBufferRange(GL_UNIFORM_BUFFER, settingsBindingPoint, g_SettingsUBO, 0,
					  4 * sizeof(uint32_t));
	glBindBuffer(GL_UNIFORM_BUFFER, 0);

	// TODO: Animation stuff is not global to all shaders. That is why we check
	// for shader flags here.
	...
	if (shaderFeatureBits & SHADER_FEATURE_MODEL_ANIMATION_BIT) {
		...
		GLuint paletteBindingPoint =
			g_PaletteBindingPoint++; // If a UBO is not being shared between
									 // shaders we need separate binding points
									 // to these buffers
		m_PaletteUniformIndex =
			glGetUniformBlockIndex(m_ShaderProgram, "Palette");
		...
		glUniformBlockBinding(m_ShaderProgram, m_PaletteUniformIndex,
							  paletteBindingPoint);

		glGenBuffers(1, &m_PaletteUBO);
		glBindBuffer(GL_UNIFORM_BUFFER, m_PaletteUBO);
		glBufferData(GL_UNIFORM_BUFFER, MAX_BONES * sizeof(glm::mat4), nullptr,
					 GL_DYNAMIC_DRAW);
		glBindBufferRange(GL_UNIFORM_BUFFER, paletteBindingPoint, m_PaletteUBO,
						  0, MAX_BONES * sizeof(glm::mat4));
		glBindBuffer(GL_UNIFORM_BUFFER, 0);
	}
...
void Shader::Activate() const { glUseProgram(m_ShaderProgram); }

GLuint Shader::Program() const { return m_ShaderProgram; }
...
void Shader::SetMatrixPalette(glm::mat4 *palette, uint32_t numMatrices) const {
	glBindBuffer(GL_UNIFORM_BUFFER, m_PaletteUBO);
	glBufferSubData(GL_UNIFORM_BUFFER, 0, numMatrices * sizeof(glm::mat4),
					palette);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
}
...
void Shader::SetShaderSettingBits(uint32_t bits) {
	...
	glBindBuffer(GL_UNIFORM_BUFFER, g_SettingsUBO);
	glBufferSubData(GL_UNIFORM_BUFFER, 0, 4 * sizeof(uint32_t),
					(void *)&settings);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
}
...
void Shader::InitGlobalBuffers() {
	...
	glBindBuffer(GL_UNIFORM_BUFFER, g_SettingsUBO);
	glBufferData(GL_UNIFORM_BUFFER, 4 * sizeof(uint32_t), nullptr,
				 GL_DYNAMIC_DRAW);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
}

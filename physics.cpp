...
void phys_AddBody(Body *body) { g_Bodies.push_back(body); }
...
void phys_ResolveContact(Contact &contact) {
	...
	float impulseJ = -(1.0f + combinedElasticity) * glm::dot(vab, normal) /
					 (invMassA + invMassB);
	...
	glm::vec3 separationDistance =
		contact.ptOnB_WorldSpace - contact.ptOnA_WorldSpace;

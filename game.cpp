...
Game::Game(std::string exePath, hkdInterface *interface, IRender *renderer) {
	...
	m_ExePath = exePath; // NOLINT(*-unnecessary-value-param)
}
...
void Game::Init() {
	...
	// TODO: Sane loading of Maps based on path
...
bool Game::RunFrame(double dt) {
	...
	glm::vec3 forward = glm::rotate(orientation, glm::vec3(0.0f, -1.0f, 0.0f));
	// -1 because the model is facing -1 (Outside the screen)
	...
	Line velocityDebugLine = {ec.center, ec.center + 200.0f * m_Player.velocity};

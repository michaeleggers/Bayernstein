...
void Game::Init() {
	...
	glm::vec4 triColor =
		glm::vec4(RandBetween(0.0f, 1.0f), RandBetween(0.0f, 1.0f),
				  RandBetween(0.0f, 1.0f), 1.0f);
	for (auto mapPoly : tris) {
		Vertex A = {glm::vec3(mapPoly.vertices[0].x, mapPoly.vertices[0].y,
							  mapPoly.vertices[0].z)};
		Vertex B = {glm::vec3(mapPoly.vertices[1].x, mapPoly.vertices[1].y,
							  mapPoly.vertices[1].z)};
		Vertex C = {glm::vec3(mapPoly.vertices[2].x, mapPoly.vertices[2].y,
							  mapPoly.vertices[2].z)};
		A.color = triColor;
		B.color = triColor;
		C.color = triColor;
		Tri tri = {A, B, C};
	...
	for (int i = 0; i < m_Player.animations.size(); i++) {
		...
		glm::vec3 scale = glm::vec3(1.0f / ec->radiusA, 1.0f / ec->radiusA,
									1.0f / ec->radiusB);
	...
	// Upload this model to the GPU. This will add the model to the model-batch
	// and you get an ID where to find the data in the batch?
...
bool Game::RunFrame(double dt) {
	...
	if (KeyPressed(SDLK_RIGHT)) {
		glm::quat rot =
			glm::angleAxis(glm::radians(-r), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	glm::vec3 forward = glm::rotate(
		...
		glm::vec3(
			0.0f, -1.0f,
			0.0f)); // -1 because the model is facing -1 (Outside the screen)
	...
	CollisionInfo collisionInfo = CollideEllipsoidWithTriPlane(
		ec, m_Player.velocity, static_cast<float>(dt) * m_World.m_Gravity,
	...
	if (collisionInfo.didCollide) {
		...
		// printf("hitpoint: %f, %f, %f\n", collisionInfo.hitPoint.x,
		// collisionInfo.hitPoint.y, collisionInfo.hitPoint.z);
	} else {

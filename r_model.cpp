...
Vertex IQMVertexToVertex(IQMVertex iqmVert, glm::vec3 bc) {
	Vertex vertex = {
		...
		.bc = bc,
		.normal =
			glm::vec3(iqmVert.normal[0], iqmVert.normal[1], iqmVert.normal[2]),
		.color = glm::vec4(iqmVert.color[0], iqmVert.color[1], iqmVert.color[2],
						   iqmVert.color[3]),
		.blendweights =
			glm::vec4(iqmVert.blendweights[0], iqmVert.blendweights[1],
...
HKD_Model CreateModelFromIQM(IQMModel *model) {
	...
	for (auto &meshe : model->meshes) {
		...
		for (int v = 0; v < iqmMesh->vertices.size(); v += 3) {
			...
			Vertex vertA =
				IQMVertexToVertex(iqmVertA, glm::vec3(1.0, 0.0, 0.0));
			Vertex vertB =
				IQMVertexToVertex(iqmVertB, glm::vec3(0.0, 1.0, 0.0));
			Vertex vertC =
				IQMVertexToVertex(iqmVertC, glm::vec3(0.0, 0.0, 1.0));
	...
	// We take the aabb of the first frame of an animation and ignore the
	// others. Might be changed later. Hopefully good enough for the start.
	...
	for (; i < model->animations.size(); i++) {
		...
		EllipsoidCollider ec =
			CreateEllipsoidColliderFromAABB(f.bbmins, f.bbmaxs);
...
static glm::mat4 PoseToMatrix(Pose pose) {
	glm::vec3 t = glm::vec3(pose.translations.x, pose.translations.y,
							pose.translations.z);
	glm::mat4 tMat =
		glm::translate(glm::mat4(1.0f), t); // TODO: Change to translation
	glm::vec3 s = glm::vec3(pose.scale.x, pose.scale.y, pose.scale.z);
	glm::mat4 sMat = glm::scale(glm::mat4(1.0f), s);
	// glm::quat r = glm::quat(pose.rotation.w, pose.rotation.x,
...
static glm::mat4 InterpolatePoses(Pose a, Pose b, float pct) {
	glm::vec3 interpTrans =
		(1.0f - pct) * a.translations + pct * b.translations;
...
void UpdateModel(HKD_Model *model, float dt) {
	if (model->type == HKD_MODEL_TYPE_ANIMATED) {
		...
		// For now, we just cylce through all animations. If the current
		// animations has reached its end, we jump to the next animation.

		if (currentFrame >= anim.firstFrame + anim.numFrames - 1) {
			// model->currentAnimIdx = (model->currentAnimIdx + 1) %
		...
		uint32_t nextFrame =
			(currentFrame + 1) % (anim.firstFrame + anim.numFrames);
		...
		for (int i = 0; i < model->numJoints; i++) {
			Pose currentPoseTransform =
				model->poses[currentFrame * model->numJoints + i];
			Pose nextPoseTransform =
				model->poses[nextFrame * model->numJoints + i];
			glm::mat4 poseMat =
				InterpolatePoses(currentPoseTransform, nextPoseTransform,
								 model->pctFrameDone / msPerFrame);
			if (currentPoseTransform.parent >= 0) {
				model->palette[i] =
					model->palette[currentPoseTransform.parent] * poseMat;
			} else {
				model->palette[i] = poseMat;
			}
		}

		// Post multiply the global transforms with the global inverse bind
		// transform to get the vertex from bindspace to local bonespace first
		// and then transform the vertex to the currents pose global bone space.
		...
		/*
		EllipsoidCollider* ec =
		&model->ellipsoidColliders[model->currentAnimIdx]; ec->center =
		model->position + glm::vec3( 0.0f, 0.0f, ec->radiusB);
		*/
	} // DONE WITH ANIMATION
...
glm::mat4 CreateModelMatrix(glm::vec3 pos, glm::quat orientation,
							glm::vec3 scale) {

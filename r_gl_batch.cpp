...
GLBatch::GLBatch(uint32_t maxVerts) {
	...
	glBufferData(GL_ARRAY_BUFFER, m_MaxVerts * sizeof(Vertex), nullptr,
				 GL_STATIC_DRAW);
	...
	glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
						  (void *)VERT_NORMAL_OFFSET);

	glEnableVertexAttribArray(4); // color
	glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex),
						  (void *)VERT_COLOR_OFFSET);

	glEnableVertexAttribArray(5); // blendindices
	glVertexAttribIPointer(5, 4, GL_UNSIGNED_INT, sizeof(Vertex),
						   (void *)VERT_BLENDINDICES_OFFSET);

	glEnableVertexAttribArray(6); // blendweights
	glVertexAttribPointer(6, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex),
						  (void *)VERT_BLENDWEIGHTS_OFFSET);
...
GLBatch::GLBatch(uint32_t maxVerts, uint32_t maxIndices) {
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_MaxIndices * sizeof(uint16_t),
...
int GLBatch::Add(Tri *tris, uint32_t numTris, bool cullFace,
				 DrawMode drawMode) {
	...
	glBindVertexArray(m_VAO);
	glBindBuffer(GL_ARRAY_BUFFER, m_VBO);
	glBufferSubData(GL_ARRAY_BUFFER, m_VertOffsetIndex * sizeof(Vertex),
					numTris * sizeof(Tri), tris);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
...
int GLBatch::Add(Vertex *verts, uint32_t numVerts, bool cullFace,
				 DrawMode drawMode) {
...
bool GLBatch::Add(Vertex *verts, uint32_t numVerts, uint16_t *indices,
				  uint32_t numIndices, int *out_offset, int *out_idxOffset,
	...
	*out_offset = m_VertOffsetIndex;
	...
	glBufferSubData(GL_ELEMENT_ARRAY_BUFFER,
					m_IndexOffsetIndex * sizeof(uint16_t),
					numIndices * sizeof(uint16_t), indices);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

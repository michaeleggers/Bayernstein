...
EllipsoidCollider CreateEllipsoidColliderFromAABB(glm::vec3 mins,
												  glm::vec3 maxs) {
...
glm::vec3 ConstructNormalToTriLineSegment(glm::vec3 a, glm::vec3 b,
...
float SignedDistancePointToPlane(glm::vec3 pt, glm::vec3 ptOnPlane,
								 glm::vec3 planeNormal) {
...
bool IsPointInTriangle(glm::vec3 point, Tri tri, glm::vec3 triNormal) {
	glm::vec3 nAB =
		ConstructNormalToTriLineSegment(tri.a.pos, tri.b.pos, triNormal);
	...
	glm::vec3 nBC =
		ConstructNormalToTriLineSegment(tri.b.pos, tri.c.pos, triNormal);
	...
	glm::vec3 nCA =
		ConstructNormalToTriLineSegment(tri.c.pos, tri.a.pos, triNormal);
...
bool CheckSweptSphereVsLinesegment(glm::vec3 p0, glm::vec3 p1,
	...
	float b = eSquaredLength * 2.0f * glm::dot(velocity, baseToVertex) -
			  2.0f * (eDotVel * eDotBaseToVertex);
	float c = eSquaredLength * (1.0f - glm::length2(baseToVertex)) +
			  eDotBaseToVertex * eDotBaseToVertex;
...
void CollideUnitSphereWithTri(CollisionInfo *ci, Tri tri) {
	...
	float t0, t1;
	if (glm::abs(velDotNormal) ==
		0.0f) { // Sphere travelling parallel to the plane or it is in the plane

		// Distance from unit sphere center to plane is greater than 1 => no
		// intersection!
		if (glm::abs(sD) >= 1.0f) {
			return;
		}
		...
		// travel the sphere along the velocity vector before a collision
		// happens.
		if (t0 > t1) {
			float tmp = t0;
			t0 = t1;
	...
	if (!embeddedInPlane) {
		...
		if (IsPointInTriangle(intersectionPoint, tri,
							  normal)) { // TODO: Rename function!
			foundCollision = true;
	...
	// ( C(t) - p )^2 = 1, that is when the sphere collided with the tri's
	// vertex. where C(t) is the current pos of the sphere on its velocity
	// vector: C(t) = basePos + t * velocity p is one of the vertices of the
	// tri.
	if (!foundCollision) {
		...
		if (CheckSweptSphereVsLinesegment(tri.a.pos, tri.b.pos, basePos,
										  velocity, t, &newT, &hitPoint)) {
			foundCollision = true;
			t = newT;
		}

		if (CheckSweptSphereVsLinesegment(tri.b.pos, tri.c.pos, basePos,
										  velocity, t, &newT, &hitPoint)) {
			foundCollision = true;
			t = newT;
		}

		if (CheckSweptSphereVsLinesegment(tri.c.pos, tri.a.pos, basePos,
										  velocity, t, &newT, &hitPoint)) {
			foundCollision = true;
			t = newT;
		}
	...
	if (foundCollision) {
		float distanceToHitpoint =
			t *
			glm::length(velocity); // furthest the sphere can travel along its
								   // velocity vector until collision happens
...
CollisionInfo CollideEllipsoidWithTriPlane(EllipsoidCollider ec,
										   glm::vec3 velocity,
										   glm::vec3 gravity,
										   TriPlane *triPlanes,
										   int triPlaneCount) {
	...
	glm::vec3 newPos = CollideEllipsoidWithTriPlaneRec(
		&ci, esBasePos, esVelocity, tris.data(), tris.size(), 0, 5);
	...
	ci.nearestDistance = 0.0f;
	ci.didCollide = false;
	newPos = CollideEllipsoidWithTriPlaneRec(&ci, newPos, esGravity,
											 tris.data(), tris.size(), 0, 5);
...
// Assume all data in ci to be in ellipsoid space, that is, a unit-sphere. Same
// goes for esBasePos.
glm::vec3 CollideEllipsoidWithTriPlaneRec(CollisionInfo *ci,
										  glm::vec3 esBasePos,
	...
	if (ci->nearestDistance >= VERY_CLOSE_DIST) {
		...
		float length =
			glm::abs(ci->nearestDistance -
					 VERY_CLOSE_DIST); // abs should not be neccessary
	...
	float distance =
		glm::dot(destinationPos - slidingPlane.p, slidingPlane.normal);
	glm::vec3 newDestinationPos =
		destinationPos - distance * slidingPlane.normal;
	...
	return CollideEllipsoidWithTriPlaneRec(ci, newBasePos, newVelocity, tris,
										   triCount, depth + 1, maxDepth);
}

...
void CollideUnitSphereWithTri(CollisionInfo* ci, Tri tri) {
	...
	if (foundCollision) {
		float distanceToHitpoint =
			t *
			glm::length(velocity); // furthest the sphere can travel along its velocity vector until collision happens
...
CollisionInfo CollideEllipsoidWithTriPlane(
	EllipsoidCollider ec, glm::vec3 velocity, glm::vec3 gravity, TriPlane* triPlanes, int triPlaneCount) {
...
glm::vec3 CollideEllipsoidWithTriPlaneRec(

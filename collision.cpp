...
void CollideUnitSphereWithTri(CollisionInfo *ci, Tri tri) {
	...
	// vector: C(t) = basePos + t * velocity p is one of the vertices of the tri.
	if (!foundCollision) {
	...
	if (foundCollision) {
		// furthest the sphere can travel along its velocity vector until collision happens
		float distanceToHitpoint = t * glm::length(velocity);

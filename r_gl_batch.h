...
class GLBatch {
  public:
	...
	bool Add(Vertex* verts, uint32_t numVerts, uint16_t* indices, uint32_t numIndices, int* out_offset,
			 int* out_idxOffset, bool cullFace = true, DrawMode drawMode = DRAW_MODE_SOLID);
	void Bind();
	void Reset();
	void Kill();

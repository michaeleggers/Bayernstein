...
enum MapVersion { QUAKE, VALVE_220 };
...
static TokenType getToken(char *c, int *pos) {
	...
	if (*cur == '{') {
		...
		result = STRING;
	} else if (*cur >= '0' && *cur <= '9' || *cur == '-') { // TODO: MINUS own token and check again if really
															// NUMBER
		result = NUMBER;
...
static Face getFace(char *c, int *pos) {
	...
	if (getToken(c, currentPos) == NUMBER) {
		...
		getToken(c, pos); // if the lookahead returned a NUMBER get the token
						  // again to advance pos.
		getNumber(c, pos);
		check(getToken(c, pos), NUMBER);
		getNumber(c, pos);
		check(getToken(c, pos), NUMBER);
...
/**
 * I assume that the grammar does not allow a brush *before* a property within
 * an entity!
 */
...
Map getMap(char *mapData, size_t mapDataLength, MapVersion mapVersion) {
	...
	int pos = 0;
	check(getToken(&mapData[0], &pos),
		  LBRACE); // Map file must start with an entity!

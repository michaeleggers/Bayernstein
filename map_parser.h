...
inline Entity* getEntityByClassName(Map* map, const std::string& className) {
	for (Entity& entity : map->entities) {
		for (Property& property : entity.properties) {
			if (property.key == "classname" && property.value == className) {
				return &entity;
			}
		}
	}

	printf("Error: No Entity found with classname: %s", className.c_str());
	assert(false && "No Entity Found");
}

inline glm::vec3 getPlayerStartPosition(Map* map) {
	Entity* infoPlayerStart = getEntityByClassName(map, "info_player_start");
	for (Property& property : infoPlayerStart->properties) {
	...
	printf("Warning: No start position found");

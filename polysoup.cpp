...
static void writePolys(const std::string &fileName,
...
bool intersectThreePlanes(MapPlane p0, MapPlane p1, MapPlane p2,
...
MapPolygon sortVerticesCCW(MapPolygon poly) {
	...
	for (size_t i = 0; i < vertCount - 1; i++) {
		glm::f64vec3 v0 =
			poly.vertices[i]; // Find next vertex to v0 with smallest angle
		...
		for (size_t j = i + 1; j < vertCount; j++) {
			...
			if (glm::dot(plane.n, test) <
				-PS_FLOAT_EPSILON) { // check if point is legal
 ...
 * Note that a lot of redundant data is generated: (v0, v1, v2), (v0, v2, v3),
 * (v0, v3, v4).
 *
